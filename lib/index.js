...
function RpcClient(opts) {
  ...
  this.port = opts.port || 57810;
  this.user = opts.user || 'user';
  this.pass = opts.pass || 'password';
...
function rpc(request, callback) {
  ...
  var errorMessage = 'FantasyGold JSON-RPC: ';
  ...
  var req = this.protocol.request(options, function(res) {
    ...
    res.on('end', function() {
      ...
      if (res.statusCode === 500 && buf.toString('utf8') === 'Work queue depth exceeded') {
        var exceededError = new Error('FantasyGold JSON-RPC: ' + buf.toString('utf8'));
...
RpcClient.callspec = {
  ...
  // address data senderAddress gasLimit amount
